...
setPath = r'Z:\\lase\\bayesian_optimization'
os.chdir(setPath)
...
def createSurface(assembly, surface_name, instance_name, catch_point):
    ...
    print(f"[createSurface] Created surface '{surface_name}' on '{instance_name}' using catch point {catch_point}.")
...
def createSurfaceBounds(assembly, surface_name, instance_name, bounds):
    ...
    if not faces:
        raise ValueError(f"[createSurfaceBounds] No faces found for surface '{surface_name}' in bounds {bounds} on instance '{instance_name}'.")
    assembly.Surface(name=surface_name, side2Faces=faces)
    print(f"[createSurfaceBounds] Created surface '{surface_name}' on '{instance_name}' with {len(faces)} face(s).")
...
def createEdge(assembly, set_name, instance_name, catch_point):
    ...
    print(f"[createEdge] Created edge set '{set_name}' on '{instance_name}' using catch point {catch_point}.")
...
def createEdgeBounds(assembly, set_name, instance_name, bounds):
    ...
    if not edges:
        raise ValueError(f"[createEdgeBounds] No edges found for set '{set_name}' in bounds {bounds} on instance '{instance_name}'.")
    assembly.Set(name=set_name, edges=edges)
    print(f"[createEdgeBounds] Created edge set '{set_name}' on '{instance_name}' with {len(edges)} edge(s).")
    return edges
...
def assignSection(model, part_name, section_name, catch_points):
    ...
    for point in catch_points:
        ...
        faces.append(face)
    if not faces:
        raise ValueError(f"[assignSection] No faces found for section assignment on part '{part_name}'.")
    all_faces = faces[0]
    for f in faces[1:]:
        all_faces += f
    ...
    print(f"[assignSection] Assigned section '{section_name}' to {len(faces)} face(s) on part '{part_name}'.")
...
def assignSectionBounds(model, part_name, section_name, bounds):
    ...
    if not faces:
        raise ValueError(f"[assignSectionBounds] No faces found for section assignment in bounds {bounds} on part '{part_name}'.")
    ...
    print(f"[assignSectionBounds] Assigned section '{section_name}' to {len(faces)} face(s) on part '{part_name}'.")


def createSet(assembly, set_name, instance_name, bounds):
    """Create a node set using faces found within a bounding box (usually not recommended)."""
    x_min, x_max, y_min, y_max, z_min, z_max = bounds
    faces = assembly.instances[instance_name].faces.getByBoundingBox(
        xMin=x_min, xMax=x_max,
        yMin=y_min, yMax=y_max,
    ...
    if not faces:
        raise ValueError(f"[createSet] No faces found for set '{set_name}' in bounds {bounds} on instance '{instance_name}'.")
    assembly.Set(name=set_name, nodes=faces)  # Note: this may not behave as expected for nodes.
    print(f"[createSet] Created face-based node set '{set_name}' with {len(faces)} face(s) on instance '{instance_name}'.")
    return faces
...
model.sketches['profileSketch'].Line(point1=(-(float(panel.width)/2), 0), point2=(float(panel.width)/2, 0))

model.sketches['profileSketch'].HorizontalConstraint(entity=model.sketches['profileSketch'].geometry[2], addUndoState=False)

model.parts['plate'].BaseShellExtrude(depth=panel.length, sketch=model.sketches['profileSketch'])
...
# === Generate surfaceList and edgeList dynamically ===

surface_list = ['plate-1', 'longFlange-2', 'transFlange-1']
for _ in range(panel.num_transverse):
    surface_list.append('transStiff-1')

edge_list = ['longStiff-2', 'longStiff-2']
for _ in range(panel.num_transverse):
    edge_list.append('longStiff-2')
for _ in range(panel.num_transverse):
    edge_list.append('longFlange-2')
edge_list += ['transStiff-1', 'transStiff-1']

# === Construct bounding boxes ===

t_offset = 0.01
bounds = [
    [-panel.width / 2, panel.width / 2, -t_offset, t_offset, -panel.length / 2, panel.length / 2],  # plate
    [-panel.width / 2, panel.width / 2, panel.h_longitudinal_web - t_offset, panel.h_longitudinal_web + t_offset, -panel.length / 2, panel.length / 2],  # longFlange
    [-panel.width / 2, panel.width / 2, panel.h_transverse_web - t_offset, panel.h_transverse_web + t_offset, -panel.length / 2, panel.length / 2]  # transFlange
]

step_size = 0
stiffener_z = -panel.length / 2
for i in range(panel.num_transverse):
    step_size = (panel.length / (2 * panel.num_transverse)) if i == 0 else (panel.length / panel.num_transverse)
    stiffener_z += step_size
    bounds.append([
        -panel.width / 2, panel.width / 2,
        -t_offset, panel.h_transverse_web + t_offset,
        stiffener_z - t_offset, stiffener_z + t_offset
    ])

# === Index lookup ===

surface_index = [0, 1, 2] + [3 + i for i in range(panel.num_transverse)]

edge_index = [0, 1]
for _ in range(2):  # for both longStiff and longFlange sets
    for i in range(panel.num_transverse):
        edge_index.append(3 + i)
edge_index += [0, 2]  # transStiff, plate

constraint_index = edge_index

# === Create surface and edge sets ===

for i, surface_name in enumerate(surface_list):
    createSurfaceBounds(assembly, f"surf-{i}", surface_name, bounds[surface_index[i]])

for i, edge_name in enumerate(edge_list):
    createEdgeBounds(assembly, f"edge-{i}", edge_name, bounds[edge_index[i]])
...
for i in range(len(constraint_index)):
    model.Tie(
        name=f"Constraint{i}",
        main=assembly.surfaces[f"surf-{constraint_index[i]}"],
        secondary=assembly.sets[f"edge-{i}"],
        adjust=ON,
...
# === Define bounding boxes for BC regions ===

bc_bounds = [
    [panel.width / 2 - t_offset, panel.width / 2 + t_offset, -t_offset, panel.h_transverse_web + t_offset, -panel.length / 2, panel.length / 2],  # +X face
    [-panel.width / 2 - t_offset, -panel.width / 2 + t_offset, -t_offset, panel.h_transverse_web + t_offset, -panel.length / 2, panel.length / 2],  # -X face
    [-panel.width / 2, panel.width / 2, -t_offset, panel.h_transverse_web + t_offset, panel.length / 2 - t_offset, panel.length / 2 + t_offset],  # +Z face
    [-panel.width / 2, panel.width / 2, -t_offset, panel.h_transverse_web + t_offset, -panel.length / 2 - t_offset, -panel.length / 2 + t_offset]  # -Z face
]

bc_instance_sets = [
    ['transFlange-1', 'transStiff-1', 'plate-1'],  # For +/- X
    ['longFlange-2', 'longStiff-2', 'plate-1']     # For +/- Z
]
...
model.Pressure(amplitude=UNSET, createStepName='Step-1', distributionType=UNIFORM, field='', magnitude=float(panel.pressure_magnitude), name='Load-1', region=assembly.surfaces['surf-0'])

# Apply a random patch load to the surface
# Create a random set of position locations

LengthBound = 2
WidthBound = 2
patchSize = 1

# random_x = random.uniform(-WidthBound, WidthBound)
# random_z = random.uniform(-LengthBound, LengthBound)

# xMin=bounds[0], xMax=bounds[1], yMin=bounds[2], yMax=bounds[3], zMin=bounds[4], zMax=bounds[5]
# createSet(assembly, "Load-1", "plate-1", [random_x - patchSize/2, random_x + patchSize / 2, float(panel.t_plate) - tOffset, float(panel.t_plate) + tOffset, random_z - patchSize / 2, random_z + patchSize / 2])
# model.Pressure(amplitude=UNSET, createStepName='Step-1', distributionType=UNIFORM, field='', magnitude=float(panel.pressure_magnitude), name='Load-1', region=Region(nodes=assembly.sets['Load-1']))

# # Subset of faces using bounding box
# # faces_in_patch = createSet(assembly, "Load-1", "plate-1", [random_x - patchSize/2, random_x + patchSize / 2, float(panel.t_plate) - tOffset, float(panel.t_plate) + tOffset, random_z - patchSize / 2, random_z + patchSize / 2])
# part = model.parts['plate']
# instance = assembly.instances['plate-1']

# bounds = [random_x - patchSize/2, random_x + patchSize / 2, float(panel.t_plate) - tOffset, float(panel.t_plate) + tOffset, random_z - patchSize / 2, random_z + patchSize / 2]
# faces_in_patch = part.faces.getByBoundingBox(xMin=bounds[0], xMax=bounds[1], yMin=bounds[2], yMax=bounds[3], zMin=bounds[4], zMax=bounds[5])

# if faces_in_patch:
#     part.Surface(name='Load-1', side1Faces=faces_in_patch)
# else:
#     raise RuntimeError("No faces found in bounding box - check mesh density and coordinates.")

# model.Pressure(
#     name='Load-1',
#     createStepName='Step-1',
#     region=instance.surfaces['Load-1'],
#     magnitude=float(panel.pressure_magnitude),
#     distributionType=UNIFORM
# )

# # Create a new surface from those faces
# assembly.Surface(name='Load-1', side1Faces=faces_in_patch)

# # Apply pressure load to that surface
# model.Pressure(
#     name='Load-1',
#     createStepName='Step-1',
#     region=assembly.surfaces['Load-1'],
#     magnitude=float(panel.pressure_magnitude),
#     distributionType=UNIFORM
# )
...
# write_trial_ndjson(panel_output, path="temp\\output.jsonl")
